...
| P4 / P5 Perception | `IMPLEMENTED` — gate `OPEN` pending recordings | `data/eval/2026-09-12/p4_perception_status.json`: 59 synthetic-frame tests, runtime camera path verified with a synthetic video, scorer proven end to end (`eval_perception.py --synthetic`); the real gate runs on the 12 annotated clips + 30 min normal prep via `scripts/eval_perception.py` (`runbook_B.md` §Field procedure) |
